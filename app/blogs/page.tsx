          ...
          <div className="grid max-[550px]:grid-cols-1 grid-cols-2 xl:grid-cols-3 xl:grid-rows-3 gap-4 2xl:gap-6">
            {blogs.map((blog: BlogProps) => (
              <Link
                href={blog.link}
                key={blog.id}
                className={`relative overflow-hidden bg-light hover:bg-light/0 transition-colors duration-300 ease-in-out xl:first:row-span-3 xl:nth-[2]:row-span-3 group ${
                  blog.id === 1 || blog.id === 2
                    ? "flex flex-col p-3 md:p-4 rounded-lg sm:rounded-xl xl:rounded-2xl"
                    : "flex flex-col p-3 md:p-4 rounded-lg sm:rounded-xl xl:grid xl:grid-cols-12 xl:items-center xl:gap-4 xl:rounded-lg xl:p-3"
                }`}
              >
                <div
                  className={`relative z-10 overflow-hidden max-md:max-h-[172px] ${
                    blog.id === 1 || blog.id === 2
                      ? "rounded-lg sm:rounded-xl h-full w-full"
                      : "rounded-lg sm:rounded-xl xl:rounded-lg h-full w-full xl:col-span-4"
                  }`}
                >
                  <Image
                    src={blog.image}
