	...
	return (
		<section className="py-12 sm:py-16 lg:py-24 2xl:py-[120px]">
			<div className="container mx-auto px-4 lg:px-6">
				<div className="grid grid-cols-2 gap-6 xl:gap-10 2xl:gap-20">
					<div className="">
						<TitleComponent
					...
					<div className="flex flex-col gap-6">
						{faqs.map((faq: FaqProps) => (
							<button
								...
								className={`relative grid ${openAccordion === faq.id ? "grid-rows-[0.1fr_1fr]" : "grid-rows-[0.1fr_0fr]"} overflow-hidden bg-light px-7 py-6 rounded-xl border border-black/15 cursor-pointer transition-all duration-300`}
							>
								<div className="flex items-center justify-between gap-10 cursor-pointer relative z-20">
									<div className="flex items-center gap-4">
										<span
											className={`text-2xl font-semibold ${openAccordion === faq.id ? "text-white" : "text-dark/30"} transition-colors duration-300 ease-in-out`}
										>
											{faq.number}
										</span>
										<h3
											className={`text-2xl font-semibold ${openAccordion === faq.id ? "text-white" : "text-dark"} transition-colors duration-300 ease-in-out`}
										>
											{faq.title}
										</h3>
									</div>
									<div
										className={`size-7 flex items-center justify-center rounded-sm ${openAccordion === faq.id ? "bg-white" : "bg-accent-light"} transition-colors duration-300 ease-in-out`}
									>
								...
								<div className={`overflow-hidden relative z-20`}>
									<div
										className={`text-base text-left pt-5 ${openAccordion === faq.id ? "text-white/90" : "text-body"} transition-colors duration-300 ease-in-out`}
									>
										{faq.text}
									</div>
								</div>
