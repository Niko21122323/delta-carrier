              ...
              <button
                ...
                className={`relative grid ${
                  openAccordion === faq.id
                    ? "grid-rows-[0.1fr_1fr]"
                    : "grid-rows-[0.1fr_0fr]"
                } overflow-hidden bg-light max-md:p-4 md:px-7 md:py-6 rounded-xl border border-black/15 cursor-pointer transition-all duration-300`}
              >
                <div className="flex items-center justify-between gap-10 cursor-pointer relative z-20">
                  <div className="flex max-sm:flex-col sm:items-center gap-4 w-full">
                    <div className="flex items-center justify-between">
                      <span
                        className={`max-[374px]:text-lg text-xl xl:text-2xl font-semibold ${
                          openAccordion === faq.id
                            ? "text-white"
                            : "text-dark/30"
                      ...
                      <div
                        className={`size-7 flex items-center justify-center rounded-sm ${
                          openAccordion === faq.id
                            ? "bg-white"
                            : "bg-accent-light"
                        } transition-colors duration-300 ease-in-out flex-shrink-0 sm:hidden`}
                      >
                        <IoChevronForwardOutline
                          className={`${
                            openAccordion === faq.id
                              ? "rotate-90 text-accent-light"
                              : "text-white"
                          } transition-colors duration-300 ease-in-out`}
                        />
                      </div>
                    ...
                    <h3
                      className={`text-left max-[374px]:text-lg text-xl xl:text-2xl font-semibold ${
                        openAccordion === faq.id ? "text-white" : "text-dark"
                      } transition-colors duration-300 ease-in-out`}
                    >
                      {faq.title}
                    </h3>
                  ...
                  <div
                    className={`size-7 flex items-center justify-center rounded-sm ${
                      openAccordion === faq.id ? "bg-white" : "bg-accent-light"
                    } transition-colors duration-300 ease-in-out flex-shrink-0 max-sm:hidden`}
                  >
                    <IoChevronForwardOutline
                      className={`${
                        openAccordion === faq.id
                ...
                <div className={`overflow-hidden relative z-20`}>
                  <div
                    className={`text-sm sm:text-base text-left pt-2 sm:pt-5 ${
                      openAccordion === faq.id ? "text-white/90" : "text-body"
                    } transition-colors duration-300 ease-in-out`}
                  >
                    {faq.text}
                  </div>
                ...
                <div
                  className={`absolute top-0 left-0 w-full h-full accordion-gradient ${
                    openAccordion === faq.id ? "opacity-100" : "opacity-0"
